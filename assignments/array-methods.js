...
//parse array for first name and add it to the new array
//parse array for last name and add it to the new array.
...
allCaps = runners.map(runners => runners.first_name.toUpperCase());
...
//need to use a filtering method as a callback.
...
//Gigashots has been indicted recently in some shady activities and the organizers want to remove the runners and refund donations. 
//Write a method to highlight the runners from Gigashots and return them into a new array.

let companyArray = [];

companyArray = runners.filter(runners => runners.company_name === "Gigashots");
...
//Collect all company names into a list and capitalize them all for inclusion into a Thank You graphic. Remove duplicates.

allCapsCompany = runners.map(runners => runners.company_name.toUpperCase());

let sponsorSet = new Set(allCapsCompany);

console.log([...sponsorSet]);

//This one took me forever, but I'm also proud of this one. 


// Problem 3

//Collect all donations below $150 and list IDs and companies of donors.

//What I want involves two operations. 1) use donations to identify all donations below $150. 2) Use the company name + id to fulfill the rest of the challenge.

let lowDonation = [];

runners.forEach((element) => {
    if(element.donation <= 150) {
        lowDonation.push(`${element.id} ${element.company_name} ${element.donation}`)
    }
});

console.log(lowDonation);
